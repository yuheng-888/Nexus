...
describe("preload runtime contract", () => {
  it("exposes marketplace and api namespaces used by the renderer", async () => {
    ...
    expect(((api.scripts as { discover?: unknown; run?: unknown })?.discover)).toEqual(expect.any(Function));
    expect(((api.scripts as { discover?: unknown; run?: unknown })?.run)).toEqual(expect.any(Function));
